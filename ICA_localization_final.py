...
# ---------------------- Source Localization & ROI Extraction ----------------------
...
# --- Add this before your for epo_file in epoch_files: loop ---

parc = config["source_localization"]["parcellation"]
hemi = config["source_localization"]["hemi"]
...
# Loop through saved epoch files for inverse modeling
epoch_files = [f for f in os.listdir(config["paths"]["epochs_dir"]) if f.endswith("_epo.fif")]
for epo_file in epoch_files:
    epo_path = os.path.join(config["paths"]["epochs_dir"], epo_file)
    raw_id = epo_file.replace("_epo.fif", "")
    print(f"🔍 Source localization for {raw_id}...")
    ...
    inv_op = make_inverse_operator(epochs.info, fwd, cov)

    # Apply inverse to all epochs
    stcs = apply_inverse_epochs(epochs, inv_op, lambda2=1. / 9., method='dSPM', pick_ori="normal")

    # Extract parcel-wise time series
    parcel_ts = []
    for stc in stcs:
        label_ts = mne.extract_label_time_course(stc, labels, inv_op['src'], mode="mean")
        parcel_ts.append(label_ts)

    # Save as .npy (shape: n_epochs, n_parcels, n_times)
    parcel_ts = np.array(parcel_ts)
    save_path = os.path.join(config["paths"]["features_dir"], f"source_{raw_id}.npy")
    np.save(save_path, parcel_ts)
    print(f"✅ Saved source time series: {save_path}")
